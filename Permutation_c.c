/*Copyright (c) 2022 Yuki Tanaka
 Released under the MIT license
 https://github.com/kwdlab/tanaka.yuki/blob/kuwakado.hidenori/LICENCE
 Original Code: https://sites.google.com/site/photonhashfunction/design
*/
/* Thanks to Chien-Ning Chen for correcting a bug in previous version
 *
 * This is the simple implementation with table lookup
 * The fastest implementation is yet to be posted.
 *
 * Last Modified:04 December 2013
 */
 #ifndef __PHOTON_H_
 #define __PHOTON_H_

 #include <stdint.h>

 #define ROUND			12
 #define min(x,y) ((x)<(y)?(x):(y))
 #define max(x,y) ((x)>(y)?(x):(y))

 #define D				6
 #define RATE			32
 #define RATEP			32
 #define DIGESTSIZE	256

 typedef uint8_t	byte;
 typedef uint32_t	u32;
 typedef uint64_t	u64;

 typedef u64 tword;
 typedef uint64_t CWord; // LSB for the first cell of the column

 typedef struct{
 	u64 h;
 	u64 l;
 }u128; // state word

 #endif /*  end of photon.h */

#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <math.h>
//#include "photon.h"
#define S				8
const byte ReductionPoly = 0x1b;

const byte WORDFILTER = ((byte) 1<<S)-1;

/* to be completed for one time pass mode */
unsigned long long MessBitLen = 0;

const byte RC[D][12] = {
	{1, 3, 7, 14, 13, 11, 6, 12, 9, 2, 5, 10},
		{0, 2, 6, 15, 12, 10, 7, 13, 8, 3, 4, 11},
		{2, 0, 4, 13, 14, 8, 5, 15, 10, 1, 6, 9},
		{6, 4, 0, 9, 10, 12, 1, 11, 14, 5, 2, 13},
		{7, 5, 1, 8, 11, 13, 0, 10, 15, 4, 3, 12},
		{5, 7, 3, 10, 9, 15, 2, 8, 13, 6, 1, 14}
};

const byte MixColMatrix[D][D] = {
	{  2,   3,   1,   2,   1,   4},
		{  8,  14,   7,   9,   6,  17},
		{ 34,  59,  31,  37,  24,  66},
		{132, 228, 121, 155, 103,  11},
		{ 22, 153, 239, 111, 144,  75},
		{150, 203, 210, 121,  36, 167}
};

byte sbox[256] = {
	
};

byte FieldMult(byte a, byte b)
{
	byte x = a, ret = 0;
	int i;
	for(i = 0; i < S; i++) {
		if((b>>i)&1) ret ^= x;
		if((x>>(S-1))&1) {
			x <<= 1;
			x ^= ReductionPoly;
		}
		else x <<= 1;
	}
	return ret&WORDFILTER;
}

void AddKey(byte state[D][D], int round)
{
	int i;
	for(i = 0; i < D; i++)
		state[i][0] ^= RC[i][round];
}

void SubCell(byte state[D][D])
{
	int i,j;
	for(i = 0; i < D; i++)
		for(j = 0; j <  D; j++)
			state[i][j] = sbox[state[i][j]];
}

void ShiftRow(byte state[D][D])
{
	int i, j;
	byte tmp[D];
	for(i = 1; i < D; i++) {
		for(j = 0; j < D; j++)
			tmp[j] = state[i][j];
		for(j = 0; j < D; j++)
			state[i][j] = tmp[(j+i)%D];
	}
}

void MixColumn(byte state[D][D])
{
	int i, j, k;
	byte tmp[D];
	for(j = 0; j < D; j++){
		for(i = 0; i < D; i++) {
			byte sum = 0;
			for(k = 0; k < D; k++)
				sum ^= FieldMult(MixColMatrix[i][k], state[k][j]);
			tmp[i] = sum;
		}
		for(i = 0; i < D; i++)
			state[i][j] = tmp[i];
	}
}
/*
������������������������������������������������������������������������������������
PERMUTATION
������������������������������������������������������������������������������������
*/
void Permutation(byte* state_in0,byte* state_in1,byte* state_in2,byte* state_in3,byte* state_in4,byte* state_in5, int R) //R=ROUND
{
    byte state[D][D];
	int i,j;
	//int型の一次元配列で受け取ったstateを二次元配列のbyte型に変換
    for(j=0;j<D;j++){
        state[0][j]=state_in0[j];
        state[1][j]=state_in1[j];
        state[2][j]=state_in2[j];
        state[3][j]=state_in3[j];
        state[4][j]=state_in4[j];
        state[5][j]=state_in5[j];
    }
	for(i = 0; i < R; i++) {
		AddKey(state, i); //PrintState(state);
		SubCell(state); //PrintState(state);
		ShiftRow(state); //PrintState(state);
		MixColumn(state);

		//PrintState(state);
	}
    for(j=0;j<D;j++){
        state_in0[j]=state[0][j];
        state_in1[j]=state[1][j];
        state_in2[j]=state[2][j];
        state_in3[j]=state[3][j];
        state_in4[j]=state[4][j];
        state_in5[j]=state[5][j];
    }
}
